...
object SupabaseService {
    ...
    suspend fun clearAllUserData(userId: String) {
        clearUserTable("sync_statistic", userId)
        clearUserTable("sync_dictionary", userId)
        clearUserTable("sync_achievements", userId)
    }

    private suspend fun clearUserTable(tableName: String, userId: String) {
        val result = WordyApplication.supabaseClient.from(tableName)
            .delete {
                filter {
                    eq("user_id", userId)
                }
        ...
        if (result != null) {
            Log.e("Supabase", "Ошибка при очистке таблицы $tableName: ${result}")
        }
    }
}