...
class GuessedInModeCondition(private val mode: GameMode) : AchievementCondition {
    override fun isSatisfied(trigger: AchievementTrigger, stats: UserStats) =
...
class NewDictionaryWord : AchievementCondition {
    override fun isSatisfied(trigger: AchievementTrigger, stats: UserStats) =
        trigger is AchievementTrigger.GameFinishedTrigger && trigger.isWin
} // обновление словаря
...
class DummyCondition : AchievementCondition {
    override fun isSatisfied(trigger: AchievementTrigger, stats: UserStats): Boolean = false
} // заглушка